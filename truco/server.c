...
// PROFESSOR O CODIGO TEM ERROS SIM, TENTAMOS DE TUDO PRA RESOLVER O B.O,
//SE ARREPENDEMO DE TER ESCOLHIDO TRUCO, TINHA MIL OUTROS JOGOS MAIS FACEIS 
//PRA GENTE FAZER, MAS VAMOS MANDAR ASSIM MESMO, ESPERO QUE ENTENDA, OBRIGADO
...
int validarEscolhaCarta(Jogador *jogador)
{
    ...
    while (1)
    {
        printf("Jogador 1, escolha uma carta (1-3): ");
...
int main()
{
    ...
    while (1)
    {
        ...
        while (rodada <= 3 && jogadores[0].rodadaGanha < 2 && jogadores[1].rodadaGanha < 2)
        {
            ...
            if (jogadores[0].rodadaGanha >= 2 || jogadores[1].rodadaGanha >= 2)
            {
                printf("\n=== TENTO FINALIZADO ===\n");
                break;
            }
            ...
            if (rodada <= 3 && jogadores[0].rodadaGanha < 2 && jogadores[1].rodadaGanha < 2)
            {
                ...
                int quemPausa = (resultado > 0) ? 0 : (resultado < 0) ? 1
                                                                      : iniciadorRodada;
        ...
        if (jogadores[0].pontos > 0 || jogadores[1].pontos > 0)
        {
            ...
            printf("\n*** Pontos ja atribuidos por corrida de truco ***\n");
        }
        ...
        printf("\nPreparando nova mao com novas cartas...\n");
        iniciandoBaralho();
        reiniciarRodada(jogadores);
        distribuirMaoParaJogadores(jogadores);
        pontosRodada = 1;
